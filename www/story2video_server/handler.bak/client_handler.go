...
// 写入shots表（更新audio_url和shot_status，对齐文档audio_response映射）
if utils.DB != nil {
for key, value := range aiResp {
if len(key) >= 5 && key[:5] == "shot_" {
shotMap, ok := value.(map[string]interface{})
...
audioURL := shotMap["audio_url"].(string)
shotStatus := shotMap["shot_status"].(string)
...
if err != nil {
log.Printf("【音频生成】更新shots表失败 | 项目ID: %s | shot_id: %s | 错误: %v", projectID, shotID, err)
} else {
log.Printf("【音频生成】更新shots表成功 | 项目ID: %s | shot_id: %s", projectID, shotID)
}
}
}
}
...
// 从shots表查询分镜数据（对齐数据库字段）
rows, err := utils.DB.Query(`
SELECT id, project_id, scene_title, prompt, narration, transition, image_url, audio_url, shot_status
...
for rows.Next() {
var id, projectID, sceneTitle, prompt, narration, transition, imageURL, audioURL, shotStatus string
err := rows.Scan(&id, &projectID, &sceneTitle, &prompt, &narration, &transition, &imageURL, &audioURL, &shotStatus)
if err != nil {
...
// 从projects表查询项目数据
rows, err := utils.DB.Query(`
SELECT project_id, user_id, raw_story, style, project_status, create_time
FROM projects WHERE user_id=?
`, userID)
if err != nil {
log.Printf("【查询项目】数据库查询失败 | 用户ID: %s | 错误: %v", userID, err)
c.JSON(http.StatusInternalServerError, gin.H{
"code": 500,
"data": nil,
...
for rows.Next() {
var projectID, userID, rawStory, style, projectStatus, createTime string
err := rows.Scan(&projectID, &userID, &rawStory, &style, &projectStatus, &createTime)
if err != nil {
log.Printf("【查询项目】数据扫描失败 | 错误: %v", err)
continue
}
